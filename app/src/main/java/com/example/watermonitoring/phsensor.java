...
public class phsensor extends AppCompatActivity implements SensorEventListener {
...
//        navigationView.setNavigationItemSelectedListener(MainActivity.this);
        ...
        ref.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                meter.setValue((ph));
                meter2.setValue(ph);
                value = Integer.valueOf(ph);
                String tr=String.valueOf(tur);
                String Ph=String.valueOf(distance);
                String dt=String.valueOf(snapshot.child(espnum).child("PH_Condition").getValue(Integer.class));
                ...
                ref.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        for (DataSnapshot snapshot1 : snapshot.getChildren()) {
                            ...
                            ref.child("Week").child(val).child("PH").child(dt).setValue(distance);
                            ref.child("Week").child(val).child("Turbidity").child(tr).setValue(tur);
                            ref.child("Week").child(val).child("Distance").child(Ph).setValue(ph);
                        }
                    }
        ...
        if (mAccelerometer != null) {
            mSensorManager.registerListener(phsensor.this, mAccelerometer, SensorManager.SENSOR_DELAY_GAME);
        }
...
//            data.addEntry(new Entry(set.getEntryCount(), (float) (Math.random() * 80) + 10f), 0);
            data.addEntry(new Entry(set.getEntryCount(), distance + 0), 0);
    ...
    protected void onDestroy() {
        mSensorManager.unregisterListener(phsensor.this);
        thread.interrupt();
        super.onDestroy();
    }
...
//
    ...
    private void sendNotification(String message, String token, String title) {
        try {
            RequestQueue requestQueue = Volley.newRequestQueue(phsensor.this);
...
//            userDetail.put("receiverImage", receiverImage);
    ...
    public void move(MenuItem item) {
        startActivity(new Intent(getApplicationContext(),phsensor2.class));
